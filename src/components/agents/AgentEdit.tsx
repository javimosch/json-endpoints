...
export function AgentEdit({ agentId, onClose }: { agentId: number; onClose: () => void }) {
  ...
  const [snapshots] = useState<Snapshot[]>([
    {
      ...
      changes: "Updated agent description"
    },
    {
      id: "2",
      timestamp: new Date(2024, 0, 28, 15, 45),
      changes: "Changed model to GPT-4"
    },
    {
      id: "3",
      timestamp: new Date(2024, 0, 28, 16, 20),
      changes: "Added Web Search tool"
    }
  ]);
  ...
  const handleRevert = (snapshotId: string) => {
    ...
    toast({
      ...
      description: "The agent has been restored to the selected version."
    });
  };
